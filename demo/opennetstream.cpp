...
void OpenNetStreamPrivate::init()
{
    ...
    if (m_library->load()) {
        ...
        m_apis.pOpenSDK_StartDownloadCloud   = (EZSDK_StartDownloadCloudFile)m_library->resolve("OpenSDK_StartDownloadCloudFile");
        m_apis.pOpenSDK_StopDownloadCloud    = (EZSDK_StopDownloadCloudFile )m_library->resolve("OpenSDK_StopDownloadCloudFile");
        ...
        m_apis.pOpenSDK_StartP2PDownload     = (EZSDK_StartP2PDownload    )m_library->resolve("OpenSDK_StartP2PDownload");
        m_apis.pOpenSDK_StopP2PDownload      = (EZSDK_StopP2PDownload     )m_library->resolve("OpenSDK_StopP2PDownload");
        m_apis.pOpenSDK_GetP2PDeviceInfo     = (EZSDK_GetP2PDeviceInfo    )m_library->resolve("OpenSDK_Data_GetP2PDeviceInfo");
...
int OpenNetStream::startDownloadCloud(const QString& session, const QString& devSerial, const int iChannelNo, const QString& safekey, const QString& startTime, const QString& stopTime)
{
    Q_D(OpenNetStream);
    return d->m_apis.pOpenSDK_StartDownloadCloud(session.toStdString().c_str(), devSerial.toStdString().c_str(), 
        iChannelNo, safekey.toStdString().c_str(), startTime.toStdString().c_str(), stopTime.toStdString().c_str());
}

int OpenNetStream::stopDownloadCloud(const QString& session)
{
    Q_D(OpenNetStream);
    return d->m_apis.pOpenSDK_StopDownloadCloud(session.toStdString().c_str());
}
...
int OpenNetStream::startP2PDownload(const QString& session, const QString& devSerial, const int iChannelNo, const QString& safekey, const QString& startTime, const QString& stopTime)
{
    Q_D(OpenNetStream);
    return d->m_apis.pOpenSDK_StartP2PDownload(session.toStdString().c_str(), devSerial.toStdString().c_str(), 
        iChannelNo, safekey.toStdString().c_str(), startTime.toStdString().c_str(), stopTime.toStdString().c_str());
}

int OpenNetStream::stopP2PDownload(const QString& session)
{
    Q_D(OpenNetStream);
    return d->m_apis.pOpenSDK_StopP2PDownload(session.toStdString().c_str());
}
...
int OpenNetStream::getP2PDeviceInfo(const QString& accessId, const QString& devSerial, void** pBuf, int* length)
{
    Q_D(OpenNetStream);
    if (d->m_apis.pOpenSDK_GetP2PDeviceInfo) {
        return d->m_apis.pOpenSDK_GetP2PDeviceInfo(accessId.toStdString().c_str(), devSerial.toStdString().c_str(), pBuf, length);
    }
    return -1;
}
