 ...
 *  \sa
 */
typedef void (__stdcall *EZSDK_SetAuthAddr)(const char* szAuthAddr);

/**
 *  \brief  设置新的平台地址
 *  \sa
 */
typedef void (__stdcall *EZSDK_SetPlatformAddr)(const char* sszPlatform);

/**
 *  \brief  设置新的Appkey
 *  \sa
 */
typedef OPENSDK_RESULT (__stdcall *EZSDK_SetAppID)(const char* szAppID);
 ...
 *  \brief 设置回放倍速
 *  \param[in] szSessionId 会话Id
 *  \param[in] iScale 回放倍速
 *  \return 0表示成功，非0表示失败
 *	\sa OpenSDK_SetPlayBackScale()
 *  \note
 *  该接口只能在OpenSDK_StartPlayBack() 调用之后才能调用
 */
typedef OPENSDK_RESULT (__stdcall *EZSDK_SetPlayBackScale)(const char* szSessionId,int iScale, const char* szOsdTime);
...
typedef OPENSDK_RESULT (__stdcall *EZSDK_StartDownloadCloudFile)(const char* szSessionId, const char* szDevSerial, const int iChannelNo, const char* szSafeKey, const char* szStartTime, const char* szStopTime);

typedef OPENSDK_RESULT (__stdcall *EZSDK_StopDownloadCloudFile)(const char* szSessionId);
...
typedef OPENSDK_RESULT (__stdcall *EZSDK_StartP2PDownload)(const char* szSessionId, const char* szDevSerial, const int iChannelNo, const char* szSafeKey, const char* szStartTime, const char* szStopTime);

typedef OPENSDK_RESULT (__stdcall *EZSDK_StopP2PDownload)(const char* szSessionId);

typedef OPENSDK_RESULT (__stdcall *EZSDK_GetP2PDeviceInfo)(const char* szAccessToken, const char* szDeviceSerial, void** pBuf, int* iLength);
 ...
 *  时间格式为：yyyy-MM-dd hh:mm:ss
 */
typedef OPENSDK_RESULT (__stdcall *EZSDK_StartSearchExtend)(const char* szSessionId, const char* szDevSerial, const int iChannelNo, const char* szStartTime, const char* szStopTime, const int iSearchType);
/**
 *  \brief 获取OSD时间
 *  \param[in]  szSessionId 会话Id
...
struct tagNETSTREAMAPI
{
    ...
    EZSDK_SetAuthAddr             pOpenSDK_SetAuthAddr;
	EZSDK_SetPlatformAddr         pOpenSDK_SetPlatformAddr;
    EZSDK_SetAppID                pOpenSDK_SetAppID;
    ...
    EZSDK_SetPlayBackScale        pOpenSDK_SetPlayBackScale;
    EZSDK_StopPlayBackEx          pOpenSDK_stopPlayBack;
    EZSDK_StartDownloadCloudFile  pOpenSDK_StartDownloadCloud;
    EZSDK_StopDownloadCloudFile   pOpenSDK_StopDownloadCloud;
    EZSDK_StartDownload           pOpenSDK_StartDownload;
    EZSDK_StopDownload            pOpenSDK_StopDownload;
    EZSDK_StartP2PDownload        pOpenSDK_StartP2PDownload;
    EZSDK_StopP2PDownload         pOpenSDK_StopP2PDownload;
    EZSDK_GetP2PDeviceInfo        pOpenSDK_GetP2PDeviceInfo;
    EZSDK_SetDataCallBack         pOpenSDK_SetDataCallBack;
    EZSDK_StartSearchEx           pOpenSDK_startSearch;
    EZSDK_StartSearchExtend       pOpenSDK_startSearchExtend;
