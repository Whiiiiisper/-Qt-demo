...
/** 
 *  \brief 按录像源进行搜索，支持本地录像和云存储录像搜索,搜索本地录像时如果关联NVR优先搜索NVR录像，不适用于国标设备
           异步接口，返回值只是表示操作成功，不代表搜索成功，不适用于国标设备
           如果接口返回-1, 结果根据消息回调函数msgid判断, 若msgid=INS_RECORD_SEARCH_END, 表示成功, 若msgid= INS_RECORD_SEARCH_FAILED, 表示失败
 *  \param[in] szSessionId   会话Id
 *  \param[in] szDevSerial   设备序列号
 *  \param[in] iChannelNo    设备通道号
 ...
 *  \return 0表示成功，其他值表示失败
 */
OPENSDK_API OPENSDK_RESULT CALLBACK OpenSDK_Data_GetDeviceDetailInfo(const char* szDevSerial, const char* szBizType, const char*szPlatform_id, const bool bUpdate, void** pDevDetailInfo, int* iLength);
